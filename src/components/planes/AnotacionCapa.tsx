        ...
        <>
            {feats.map((f, i) => {
                ...
                if (g.type === 'LineString') {
                    ...
                    return <Polyline key={i} positions={pos} pathOptions={{ color, weight: grosor, opacity: opacidad }} />
                }
                if (g.type === 'Polygon') {
                    const pos = (g.coordinates?.[0] || []).map((c: number[]) => [c[1], c[0]] as LatLng)
                    return <Polygon key={i} positions={pos} pathOptions={{ color, weight: grosor, opacity: opacidad, fillColor: color, fillOpacity: (p.relleno === false ? 0 : 0.25) * opacidad }} />
                }
                if (g.type === 'Point') {
                    ...
                    if (p.iconoUrl) {
                        return <Marker key={i} position={ll} interactive={false} icon={iconoMarcador(p.iconoUrl, p.iconoTam ?? 36)} />
                    }
                }
                return null
            })}
