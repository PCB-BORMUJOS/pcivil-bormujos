...
export default function EditorPlano({ edificioId, edificioNombre, planoUrl, equiposECI, onCerrar }: Props) {
  ...
  useEffect(() => {
    fetch(`/api/incendios/marcadores?edificioId=${edificioId}`)
      .then(r => r.json())
  ...
  const guardarMarcador = async () => {
    ...
    try {
      const res = await fetch('/api/incendios/marcadores', {
        ...
        body: JSON.stringify({
          edificioId, tipo: tipoSeleccionado,
