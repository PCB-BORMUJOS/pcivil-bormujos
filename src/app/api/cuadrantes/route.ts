...
export async function GET(request: NextRequest) {
  try {
    ...
    const todosUsuariosActivos = await prisma.usuario.findMany({
      // Miembros del servicio en el cuadrante: los operativos MÁS B-12 (personal
      // de administración, que aunque no es operativo sí debe figurar). Fuera solo
      // los visores (indicativos que solo consultan la app, p. ej. J0/J1) y el resto
      // de no operativos.
      where: {
        activo: true,
        NOT: { rol: { nombre: 'visor' } },
        OR: [
          { esOperativo: { not: false } },
          { fichaVoluntario: { indicativo2: 'B-12' } },
        ],
      },
      select: {
        id: true, nombre: true, apellidos: true, numeroVoluntario: true,
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const [configBaremo, configKm, configJ44] = await Promise.all([
      ...
      const esJefeServicio = fichaCheck?.indicativo2 === INDICATIVO_JEFE_SERVICIO;
      ...
      // Baremo: para el voluntariado se calcula sobre el TOTAL del día; para el
      // Jefe de Servicio (J-44) cada turno es una dieta independiente y el tramo
      // se calcula solo con las horas de ESE turno (mañana y tarde = dos dietas
      // de "más de 4h", no una de 8h). Los tramos de 8h/12h solo se fuerzan con
      // el botón del slot.
      const horasParaTramo = esJefeServicio ? horasTrabajadas : horasTotalesDia;
      const tramo = [...baremo].reverse().find(t => horasParaTramo >= (t.horasMin ?? t.minHours ?? 0));
      ...
      // Si hay otros turnos del mismo día, poner su importeDia a 0 (el baremo
      // acumulado va aquí). NO aplica a J-44: cada uno de sus turnos conserva su
      // propia dieta de "más de 4h".
