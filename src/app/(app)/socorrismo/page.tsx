...
const ESTADOS_PETICION = {
  ...
  aprobada: { color: 'bg-blue-100 text-blue-700', label: 'Aprobada', icon: Check },
  en_compra: { color: 'bg-purple-100 text-purple-700', label: 'En Compra', icon: ShoppingCart },
  recibida: { color: 'bg-green-100 text-green-700', label: 'Recibida', icon: CheckCircle },
};
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setShowNuevaPeticion(true)} className="px-4 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center gap-2 font-medium">
            <ShoppingCart size={18} />Nueva Petición
          </button>
          ...
          <button onClick={() => setShowNuevoDEA(true)} className="px-4 py-2.5 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 flex items-center gap-2 font-medium">
            <Droplet size={18} />Nuevo DEA
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
              ...
              {inventoryTab === 'stock' && (
                <div>
                  ...
                  {loading ? (
                    ...
                    <p className="text-slate-500">Tabla de artículos aquí - {articulosFiltrados.length} artículos</p>
                  )}
                </div>
              )}
              {inventoryTab === 'peticiones' && <p className="text-slate-500">Peticiones - {peticiones.length}</p>}
              {inventoryTab === 'movimientos' && <p className="text-slate-500">Movimientos</p>}
            </div>
          </div>
        )}
        ...
        {mainTab === 'deas' && (
          <div className="p-6">
            <p className="text-slate-500">Mapa de DEAs - {deas.length} DEAs totales</p>
          </div>
        )}
      </div>
    </div>
