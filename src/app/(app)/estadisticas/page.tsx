...
// El año y sus opciones son propios de esta página; el resto de piezas
// visuales viven en components/estadisticas/ui para poder reutilizarlas.
const now = new Date()
      ...
      <div className="px-6 py-6 max-w-screen-2xl mx-auto space-y-6">
        ...
        {!loading && (
          <>
              ...
              <div className="space-y-6">
                ...
                {partesPSI.length>0 && (
                  <Panel title="Partes PSI registrados">
                    {/* Antes pedía tipoIncidencia y municipio, que no existen en
                        PartePSI: las dos columnas salían siempre con un guion. */}
                    <DataTable
                      heads={['Fecha','Motivo','Lugar','Estado']}
                      rows={partesPSI.map((p:any)=>[
                        fmtDate(p.fecha||p.createdAt),
                        p.motivo||'—',
                        p.lugar||'—',
            ...
            {/* ══════════════ PARTES ══════════════ */}
            {/* Lo mismo que se ve dentro de PSI, PRF y PAS, aquí con los tres
                juntos. El año lo manda el selector de esta página. */}
            {tab==='partes' && (
              <EstadisticasPartes tipos={['PSI','PRF','PAS']} anio={year} />
            )}
