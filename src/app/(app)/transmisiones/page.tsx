...
function createMapIcon(tipo: 'repetidor' | 'base' | 'base_junta' | 'portatil' | 'custom', color: string, size: number = 40) {
  const L = typeof window !== "undefined" ? require("leaflet") : null;
  if (!L) return undefined;
  const svgIcons: Record<string, string> = {
    repetidor: '<path d="M12 3v14M8 6l4-3 4 3" stroke="white" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"/><path d="M6 10a7 7 0 0 0 2 4" stroke="white" stroke-width="1.5" fill="none" stroke-linecap="round"/><path d="M18 10a7 7 0 0 1-2 4" stroke="white" stroke-width="1.5" fill="none" stroke-linecap="round"/><circle cx="12" cy="18" r="1.5" fill="white"/>',
    base: '<rect x="5" y="11" width="14" height="8" rx="1.5" stroke="white" stroke-width="1.5" fill="none"/><path d="M12 4v7M8 7l4-3 4 3" stroke="white" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"/><circle cx="9" cy="15" r="1" fill="white"/><circle cx="15" cy="15" r="1" fill="white"/>',
    base_junta: '<path d="M12 3l-8 5h16l-8-5z" fill="white"/><rect x="5" y="9" width="14" height="1.5" fill="white"/><rect x="7" y="11" width="2" height="5" fill="white"/><rect x="11" y="11" width="2" height="5" fill="white"/><rect x="15" y="11" width="2" height="5" fill="white"/><rect x="5" y="16.5" width="14" height="2" rx="0.5" fill="white"/>',
    portatil: '<rect x="8.5" y="5" width="7" height="13" rx="1.5" stroke="white" stroke-width="1.5" fill="none"/><path d="M12 5V2" stroke="white" stroke-width="2" stroke-linecap="round"/><rect x="10" y="8" width="4" height="4" rx="0.5" fill="white"/><circle cx="12" cy="15" r="1" fill="white"/>',
    custom: '<circle cx="12" cy="11" r="4" stroke="white" stroke-width="1.5" fill="none"/><circle cx="12" cy="11" r="1.5" fill="white"/>'
  };
  const icon = svgIcons[tipo] || svgIcons.custom;
  return L.divIcon({
    className: '',
    html: '<div style="position:relative;width:'+size+'px;height:'+(size+12)+'px;filter:drop-shadow(0 3px 4px rgba(0,0,0,0.35))"><svg width="'+size+'" height="'+(size+12)+'" viewBox="0 0 40 52" xmlns="http://www.w3.org/2000/svg"><path d="M20 0C9 0 0 9 0 20c0 15 20 32 20 32s20-17 20-32C40 9 31 0 20 0z" fill="'+color+'" stroke="white" stroke-width="2.5"/><g transform="translate(8,6) scale(1)">'+icon+'</g></svg></div>',
...
export default function TransmisionesPage() {
  ...
  const cargarPeticiones = useCallback(async () => { try { const r = await fetch('/api/logistica/peticiones?area=transmisiones'); const d = await r.json(); setPeticiones(d.peticiones || []) } catch (e) { /* error silenciado */ } }, [])
  const cargarMantenimientos = useCallback(async (eqId: string) => { try { const r = await fetch(`/api/logistica?tipo=mantenimientos-equipo&equipoId=${eqId}`); const d = await r.json(); setMantenimientos(d.mantenimientos || []) } catch (e) { /* error silenciado */ } }, [])
  const cargarCiclos = useCallback(async (eqId: string) => { try { const r = await fetch(`/api/logistica?tipo=ciclos-carga&equipoId=${eqId}`); const d = await r.json(); setCiclosCarga(d.ciclos || []) } catch (e) { /* error silenciado */ } }, [])
  ...
  const handleGuardarConfigRF = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    }) }); if (r.ok) { setShowConfigRF(false); setEquipoSeleccionado(null); cargarDatos() } } catch (e) { /* error silenciado */ } finally { setSaving(false) }
  }
